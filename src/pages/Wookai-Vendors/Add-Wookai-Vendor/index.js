...
class VendorAdd extends React.Component {
  ...
  fetchVendor = async (id) => {
    let res = await httpGet(`${WOOKAI_VENDORS_ENDPOINT}/${id}`);
    this.setState({
  ...
  handleSubmit = async (e) => {
    ...
    if (enable_deliver_date == 1 && !food_format) {
      errorToaster("Please fill required fields", "Error");
      return false;
    }
  ...
  update = async () => {
    ...
    const res = await httpPut(`${WOOKAI_VENDORS_ENDPOINT}/${id}`, data);
    this.setState({
      loading: false,
    });
  ...
  createNew = async () => {
    ...
    const res = await httpPost(`${WOOKAI_VENDORS_ENDPOINT}`, data);
    this.setState({
      loading: false,
    });

    this.props.history.push('/wookaiVendors/list');
      ...
      <React.Fragment>
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs title="WookAI Suppliers" breadcrumbItem="Add" />
            {loading && <Loader />}
            <Row>
              <Col xl="12">
                <Card>
                          ...
                          <Col md="4">
                            <FormGroup>
                              ...
                              <input
                                ...
                                onChange={(event) => this.changeHandeler(event)}
                              />
                              <div className="invalid-feedback">Enter case</div>
                            </FormGroup>
                          </Col>
                        </Row>
                        <Row>

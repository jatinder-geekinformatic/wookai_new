...
class SupplierRepAdd extends React.Component {
  /**
   * component state
   */
  state = {
  ...
  handleSubmit = async (e) => {
    ...
    if (id) this.props.updateSupplierRep(this.props.history, id);
    else this.props.addSupplierRep(this.props.history);
  };

  //Form Input onChange
  changeHandeler = (event) => {
    this.props.setSupplierRep({
...
const mapStatetoProps = (state) => {
  const { supplierRep, loading } = state.SupplierRep;
  const { countries } = state.Country;
  const { states } = state.State;
  const { timeZones } = state.TimeZone;
  const {defaultProfiles} = state.DefaultProfile;

  return { supplierRep, countries, states, timeZones, loading, defaultProfiles };
};
